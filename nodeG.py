...
# IP and MAC Address
IP_Addr = "176.16.254.2"
MAC_Addr = "45:D4:01:00:00:00"
...
# Connect to Router 2
nodeG = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
ROUTER2_PORT = data['ROUTER_2']['PORT_NO']
SERVER = socket.gethostbyname("localhost")
ROUTER2_ADDR = (SERVER, ROUTER2_PORT)
nodeG.connect(ROUTER2_ADDR)
...
nodeG.send(str(newConnectionMessage).encode('utf-8'))
newConnectionReply = nodeG.recv(2048).decode('utf-8')
...
while True:
    ...
    packetReceived = nodeG.recv(2048).decode('utf-8')
    packetReceived = packetReceived.replace('\'', '"')
    packetReceived = json.loads(packetReceived)

    handle_message(packetReceived, nodeG)
