...
# Connect to NODE A
nodeE = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
NODE_A_PORT = data['NODE_A']['PORT_NO']
NODEA_ADDR = (SERVER, NODE_A_PORT)
nodeE.connect(NODEA_ADDR)
...
nodeE.send(str(newConnectionMessage).encode('utf-8'))
newConnectionReply = nodeE.recv(2048).decode('utf-8')
print(newConnectionReply)
printRoutingTable()
print(nodeE.recv(2048).decode('utf-8'))
print()

while True:

    packetReceived = nodeE.recv(2048).decode('utf-8')
    packetReceived = packetReceived.replace('\'', '"')
    packetReceived = json.loads(packetReceived)

    handle_message(packetReceived, nodeE)
