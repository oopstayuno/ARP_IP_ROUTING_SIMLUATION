...
# Connect to Router 1
nodeK = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
SERVER = socket.gethostbyname("localhost")
ROUTER1_PORT = data['ROUTER_1']['PORT_NO']
ROUTER1_ADDR = (SERVER, ROUTER1_PORT)
nodeK.connect(ROUTER1_ADDR)

ROUTING_TABLE['default']['SOCKET'] = nodeK
...
newConnectionReply = nodeK.recv(2048).decode('utf-8')
...
while True:
    ...
    packetReceived = nodeK.recv(2048).decode('utf-8')
    packetReceived = packetReceived.replace('\'', '"')
    packetReceived = json.loads(packetReceived)

    handle_message(packetReceived, nodeK)
