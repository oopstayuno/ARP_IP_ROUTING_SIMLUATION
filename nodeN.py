...
# IP and MAC Address
IP_Addr = "192.168.1.133"
MAC_Addr = "15:14:21:22:A0:20"
...
# Connect to Router 1
nodeN = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
SERVER = socket.gethostbyname("localhost")
ROUTER1_PORT = data['ROUTER_1']['PORT_NO']
ROUTER1_ADDR = (SERVER, ROUTER1_PORT)
nodeN.connect(ROUTER1_ADDR)

ROUTING_TABLE['default']['SOCKET'] = nodeN
...
newConnectionReply = nodeN.recv(2048).decode('utf-8')
...
while True:
    ...
    packetReceived = nodeN.recv(2048).decode('utf-8')
    packetReceived = packetReceived.replace('\'', '"')
    packetReceived = json.loads(packetReceived)

    handle_message(packetReceived, nodeN)
