...
# IP and MAC Address
IP_Addr = "176.16.254.4"
MAC_Addr = "BB:DD:AA:00:00:00"
...
# Connect to Router 2
nodeI = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
ROUTER2_PORT = data['ROUTER_2']['PORT_NO']
SERVER = socket.gethostbyname("localhost")
ROUTER2_ADDR = (SERVER, ROUTER2_PORT)
nodeI.connect(ROUTER2_ADDR)
...
nodeI.send(str(newConnectionMessage).encode('utf-8'))
newConnectionReply = nodeI.recv(2048).decode('utf-8')
...
while True:
    ...
    packetReceived = nodeI.recv(2048).decode('utf-8')
    packetReceived = packetReceived.replace('\'', '"')
    packetReceived = json.loads(packetReceived)

    handle_message(packetReceived, nodeI)
