...
# IP and MAC Address
IP_Addr = "176.16.254.5"
MAC_Addr = "BB:BB:AA:BB:22:00"
...
# Connect to Router 2
nodeJ = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
ROUTER2_PORT = data['ROUTER_2']['PORT_NO']
SERVER = socket.gethostbyname("localhost")
ROUTER2_ADDR = (SERVER, ROUTER2_PORT)
nodeJ.connect(ROUTER2_ADDR)
...
nodeJ.send(str(newConnectionMessage).encode('utf-8'))
newConnectionReply = nodeJ.recv(2048).decode('utf-8')
...
while True:
    ...
    packetReceived = nodeJ.recv(2048).decode('utf-8')
    packetReceived = packetReceived.replace('\'', '"')
    packetReceived = json.loads(packetReceived)

    handle_message(packetReceived, nodeJ)
